<template>
  <div>
    <div>
      ...
      >
        <div class="wrapper">
          <div v-if="data.additional_info.prepend_img">
            <div v-if="!data.additional_info.prepend_tooltip">
              <img
            ...
            <div v-else>
              <el-tooltip
                class="item"
                effect="dark"
                :content="data.additional_info.prepend_tooltip"
                placement="top-start"
              >
                <img
                  :src="selectedImages(data.additional_info.prepend_img)"
          ...
          >
            ...
            <div
              v-if="
                data.additional_info.additional_question == 'multiple_choice'
              "
            ...
            <div style="margin-left: auto; display: flex; align-items: center">
              ...
              >
                <template
                  #append
                  v-if="data.additional_info.append_text !== ''"
                >
                  <div>
      ...
      <div v-if="data.additional_info.note">
...
const validateFocusedField = (item) => {
  let label = "";
  if (item == "Available Roof Top Area") {
    label = "Rooftop Area";
  } else {
    label = item;
  }
  emits("changeError", label, false);
};
  ...
  switch (item) {
    ...
    case "Available Roof Top Area":
      ...
      if (parseInt(data.additional_info.default_option) >= 120) {
        ...
        emits("changeError", "Rooftop Area", false);
      } else {
        console.log("hello");
        emits("changeError", "Rooftop Area", true);
      }
      break;
...
const handleTariffChange = () => {
  ...
  if (inputValue.value === "" || inputValue.value <= 0) {
    ...
    emits("changeError", "Tariff Rates", true);
    return;
  }
  if (inputValue.value < 4) {
    ...
    emits("changeError", "Tariff Rates", false);
...
.lastInputContainer >>> .el-input-group__append {
  box-shadow: none;
...
.lastInputContainer >>> .el-input__wrapper {
  border: none;
  box-shadow: none;
  padding-left: 1px;
  background: none;
...
.lastInputContainer >>> .el-input-group__prepend {
  box-shadow: none;
  background-color: white;
  padding: 0;
}
