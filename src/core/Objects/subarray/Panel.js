...
export default class Panel extends BaseObject {
    constructor(stage, panelMap) {
        ...
        this.panelMesh = utils.createMesh(utils.createBufferGeometry(), this.meshMaterial);
    ...
    getPlacingInformation() {
        ...
        const modelsBelow = raycastingUtils.getAllModelsBelowVertices(
            utils.convertVectorToArray(this.panelMesh.geometry.vertices),
            this.stage,
        );
    ...
    updateVisualsBasedOnStates() {
        ...
        const isSelected = this.stage.selectionControls.getSelectedObjects().includes(this.parent);
        ...
        if (newColors !== undefined && newColors !== null) {
            utils.updateMeshWithColor(newColors.MESH_COLOR, this.panelMesh);
            utils.updateMeshWithColor(newColors.EDGE_COLOR, this.edgeMesh);
        }
    }

    updateColorToGrey() {
        utils.updateMeshWithColor(0x4A6A79, this.panelMesh);
    }

    updateColorToSelected() {
        utils.updateMeshWithColor(
    ...
    showPanel({ shouldSaveState } = { shouldSaveState: false }) {
        this.hidden = false;
    ...
    removeFromScene() {
        this.getParent().handleObjectsGroupDeletion(this.objectsGroup);
    }
